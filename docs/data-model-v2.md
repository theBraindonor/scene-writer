# Scene Writer data model — v2

## Purpose

Scene Writer persists its working data in a local SQLite database. This document is
the source of truth for version 2 of the proposed relational model; it describes the
schema but does not create a database.
...
## What concretely changed from v1

These changes are intentional schema changes, not aliases. Code, migrations, queries,
forms, API contracts, and LLM prompt builders must use the v2 names below.

| v1 | v2 | Required code update |
| --- | --- | --- |
| `story.scenario` | `story.story_brief` | Rename the column and all reads/writes. The value remains required. |
| `scene.description` | `scene.brief` | Rename the column and all reads/writes. The value remains required. |
| `scene.length` | `scene.target_length` | Rename the column and all reads/writes. The value remains optional generation guidance. |
| — | `story.generation_guideance` | Add an optional per-story text field for generation instructions beyond style. This spelling is intentional for this v2 schema. |
| — | `scene.pov_character_id` | Add an optional foreign key to `character`. It identifies the scene's point-of-view character. |
| — | `scene.desired_outcome` | Add optional text describing the desired state, decision, revelation, or complication at the end of the scene. |
| — | `continuity_snapshot` | Add one narrative-state checkpoint for each story/scene boundary that needs continuity context. |

No existing entity, relationship, constraint, or rendering-selection behavior has been
removed in v2. The `continuity_snapshot` table persists the compact, generated
scene-to-scene state separately from the human-authored story bible.
...
- Store ordered content explicitly. Future scenes belong to a story and have a stable
  position within it.
